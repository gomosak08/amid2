...
def create
  ...
  @appointment = Appointment.new(appointment_params.merge(
    status:   "Scheduled",
    end_date: Time.zone.parse(params[:appointment][:start_date]) + @package.duration.to_i.minutes
  ))

  respond_to do |format|
    format.html do
      if @appointment.save
        redirect_to admin_appointment_path(@appointment), notice: "Cita creada con éxito."
      else
    ...
    format.turbo_stream do
      ...
      format.turbo_stream do
        # This will cause Turbo to navigate to the show (confirmation) page
        redirect_to admin_appointment_path(@appointment), status: :see_other
      end
      else
        # Re-render the client-data form with errors
        render turbo_stream: turbo_stream.replace(
          "client_data_frame",
          partial: "admin/appointments/client_data_form",
          locals: {
