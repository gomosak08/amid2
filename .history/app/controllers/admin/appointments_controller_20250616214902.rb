class Admin::AppointmentsController < ApplicationController
  ...
  def available_fields
    ...
    if @doctor_id.blank? || @appointment_date.blank?
      # just show doctor/date inside the main frame
      ...
      # show doctor/date in the frame…
      streams << turbo_stream.replace(
        "appointment_form",
        partial: "admin/appointments/doctor_date_form",
        locals: { package: @package, doctors: @doctors, doctor_id: @doctor_id, appointment_date: @appointment_date }
      )
      # …and then append the available times underneath
      @available_times = fetch_available_times(Doctor.find(@doctor_id), @appointment_date, @duration)
  ...
  def create
    ...
    respond_to do |format|
      if @appointment.save
        ...
        format.html { redirect_to admin_appointments_path, notice: "Appointment booked!" }
      else
        # Re-render the available_times_form with errors
        @doctors          = Doctor.all
        @doctor_id        = params[:doctor_id]
        @appointment_date = params[:appointment_date]
        @duration         = @package.duration.to_i
