...
# app/controllers/admin/appointments_controller.rb
def create
  # Find the package to compute end_date
  ...
  respond_to do |format|
    # HTML fallback for non‐Turbo users
    format.html do
      if @appointment.save
        redirect_to admin_appointment_path(@appointment),
                    notice: "Cita creada con éxito."
      else
    ...
    format.turbo_stream do
      if @appointment.save
        # Instruct Turbo to navigate to the appointment's show page
        render turbo_stream: turbo_stream.redirect_to(
          admin_appointment_path(@appointment)
        )
      else
        # Re‐render the client data form with inline errors
        render turbo_stream: turbo_stream.replace(
          "client_data_frame",
          partial: "admin/appointments/client_data_form",
          locals: {
