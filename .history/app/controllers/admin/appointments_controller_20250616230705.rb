...
def new
      ...
      @available_times  = []
      @duration         = @package&.duration.to_i || 0

      # Step 3: fetch times only when doctor+date provided
      ...
      respond_to do |format|
        ...
        format.turbo_stream do
          ...
          # Step 2 → render doctor+date form
          if params[:package_id].present? && @doctor_id.blank?
            streams << turbo_stream.replace(
              "doctor_date_frame",
              partial: "admin/appointments/doctor_date_form",
              locals: { package: @package, doctors: @doctors, doctor_id: @doctor_id, appointment_date: @appointment_date }
            )

          # Step 3 → render time-picker form
          ...
          # Step 4 → render client-data form
...
def available_fields
  ...
  respond_to do |format|
    ...
    def create
      ...
      @appointment.end_date = @appointment.start_date + @package.duration.to_i.minutes
      ...
      unless time_slot_available?(@appointment.doctor_id, @appointment.start_date)
        flash.now[:alert] = "The selected time is no longer available."
        @doctor_id        = @appointment.doctor_id
        @appointment_date = @appointment.start_date.to_date.iso8601
        @duration         = @package.duration.to_i
      ...
      if @appointment.save
        ...
        render turbo_stream: turbo_stream.replace(
          ...
          locals: {
            ...
            doctor_id:        @appointment.doctor_id,
            appointment_date: @appointment.start_date.to_date.iso8601,
            time_slot:        @appointment.start_date.iso8601
          }
        ), status: :unprocessable_entity
      end
    end
